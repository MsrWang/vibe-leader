...
> **3.1.0 macOS 兼容设计与验收边界**
> 通用设计目标为 macOS 14+、Apple Silicon M1+ 和稳定版 CPython 3.11–3.14（最低 3.11；新安装推荐 3.14.7）；3.15+ 保持 `RUNTIME_UNVERIFIED`。2026 Mac mini（M6）仅为参考验收目标，不是已验证结论；参考配置还包括验收当日的 macOS 27 稳定补丁版本和 CPython 3.14.7。
> 3.1 使用 `Vibe-Leader-3.1.0-GitHub.zip`、`release_archive.py` 和 `SHA256SUMS.txt` 三个固定资产；生命周期状态由外部 Release 元数据和经签名或平台回读的收据建立。包内说明不自称已经发布、安装或被 Codex App 验收。[查看 3.1 边界](docs/release-3.1.0.md)和[Mac 验收步骤](docs/macos-acceptance-3.1.md)。
...
[先看在线示例](https://huggingface.co/spaces/MsrWang0112/vibe-leader) · [快速开始](#快速开始) · [查看版本下载页](https://github.com/MsrWang/vibe-leader/releases) · [3.1 边界](docs/release-3.1.0.md) · [3.0.3 说明](docs/release-3.0.3.md) · [安装与使用指南](docs/getting-started.md)

## 快速开始

### 1. 先看看怎么用

打开 [HF 在线示例](https://huggingface.co/spaces/MsrWang0112/vibe-leader)，选择一个场景，看看可以怎么提需求、Codex 应怎样处理。该页面独立显示其部署版本，生命周期状态以平台元数据和回读收据为准；四个预先编写的合成示例不运行本地证据筛选器。无需安装即可浏览，实际任务要在你自己的 Codex 中完成。
...
既有验证证据覆盖 Windows Codex Desktop 配合 Ubuntu/WSL。3.1 的设计目标为 macOS 14+、Apple Silicon M1+ 和稳定版 CPython 3.11–3.14（最低 3.11；新安装推荐 3.14.7）；2026 Mac mini（M6）仅为参考验收目标，不是已验证结论。WSL 测试不是 Mac 原生验证，Mac 技术检查和 Codex App 用户观察须按验收指南另行建立。
