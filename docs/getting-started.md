...
| 目标 | 从哪里开始 | 不必先做什么 |
|---|---|---|
| 了解基础 Skill 怎么用 | 阅读第 3–4 节的显式启动与本地小任务流程；若尚未安装，先完成第 1–2 节的兼容核验和单独授权 | 不必先接入工程委派、运行采集或模型评测 |
| 核对安装或停用/恢复 | 第 1–2、5 节；区分文件核验、宿主发现和实际使用 | 不借用维护者的旧安装或审批 |
| 开发可选工程工具 | 第 6 节，按任务需要接入身份、运行证据与委派接收 | 不把工具全套接通作为基础 Skill 启动条件 |
