...
3.0 的日常请求见[日常项目用法](project-workflow.md)，感觉方向不对时可直接使用[独立纠偏卡](deviation-correction.md)，判断修改位置与影响见[维护说明](maintenance-and-change.md)。3.0.2 修改了 Skill 的纠偏工作流：只阅读说明无需安装；要让现有 3.0.1 或更早安装获得“三行说明后继续”的轻量纠偏，需要按本页受控升级边界更新实际安装。证据与限制见[3.0.2 说明](release-3.0.2.md)，连续推进的原始变化见[3.0.1 说明](release-3.0.1.md)。
...
你需要一个能发现并显式调用本 Skill 的 Codex 宿主；源码安装器需要 Python 3.11+，Git 项目需要 Git。不需要维护者的账户配置、旧审批或私人插件库存。

当前安装器仅接受已经存在、无符号链接且已规范化的 `CODEX_HOME` 及其 `skills` 子目录。它不会替你查明 Windows/WSL 哪份配置正在生效，也不会创建另一套宿主配置。

**安装前先确认宿主的发现目录。** 本安装器固定写 `CODEX_HOME/skills`，已在 Windows Codex Desktop 配合 Ubuntu/WSL 的验证环境发现并显式加载。其他宿主请核对其当前说明和实际发现结果；若不识别此目标，停止安装，不伪造 `CODEX_HOME`、复制同名 Skill 或改写清单绕过。
